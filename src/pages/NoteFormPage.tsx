...
import {
  generateLessonNote,
  stripHtmlTags,
  type AiGeneratedLessonNote,
  type AiLessonGenerationRequest,
} from "../lib/gemini";
...
const NoteFormPage = () => {
  ...
  const [aiGeneratedDraft, setAiGeneratedDraft] =
    useState<AiGeneratedDraft | null>(null);
  ...
  const insertAiDraft = () => {
    ...
    setValue(
      "teachers_presentation",
      stripHtmlTags(aiGeneratedDraft.teachers_presentation),
    );
    ...
    fetchNoteById(id, profile.id)
      ...
      .catch(() => {
        ...
        if (draft) {
          ...
          hydratedRef.current = true;
          toast.error(
            "Unable to load from server. Your local draft is still available.",
          );
        } else {
  ...
  useEffect(() => {
    ...
    const valuesToSave = formFields.reduce<Record<string, string>>(
      (acc, field) => {
  ...
  const latestNote = async () => {
    ...
    try {
      ...
      const last = notes.find(
        (note) => note.id !== id && note.status !== "draft",
      );
      if (!last) {
  ...
  const onSubmit = async (
    ...
    try {
      if (id && existingNote) {
        ...
        toast.success(
          status === "submitted" ? "Lesson submitted" : "Draft saved",
        );
      } else {
      ...
      toast.error(
        error?.message ||
          "Unable to save note. Your work is still protected locally.",
      );
    }
  };
  ...
  return (
    <NavigationShell role="teacher">
      <div className="min-h-screen bg-slate-50 px-4 pb-24 py-6 sm:px-6 md:pb-8 lg:px-8">
        <div className="mx-auto max-w-5xl">
            ...
            <div className="flex items-center gap-4">
              ...
              <div>
                ...
                <h1 className="mt-2 text-3xl font-semibold text-slate-900">
                  {id ? "Edit lesson note" : "Create a new lesson note"}
                </h1>
              </div>
            </div>
          </div>
          <div className="rounded-[32px] border border-slate-200 bg-white p-8 shadow-soft">
            <PageHeader
              ...
              description="Complete all sections and save your work as a draft or submit for review."
            />
            <div className="mb-6 flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between">
              <p className="text-sm text-slate-600">
                Use AI to generate a structured lesson draft based on your topic
                and grade level.
              </p>
              <Button
                type="button"
                variant="secondary"
                onClick={openAiPanel}
                disabled={noteLocked || aiGenerating}
              >
            ...
            {noteLocked ? (
              <div className="rounded-2xl border border-amber-200 bg-amber-50 px-5 py-4 text-sm text-amber-900">
                This lesson note has already been submitted or approved. You
                cannot edit it further unless it is rejected.
              </div>
            ) : null}
            {isAiPanelOpen ? (
                  ...
                  <div className="mb-6 flex flex-col gap-4 sm:flex-row sm:items-start sm:justify-between">
                    <div>
                      ...
                      <p className="mt-2 text-sm text-slate-500">
                        Fill the inputs below, generate a structured note, then
                        insert it into the form for review.
                      </p>
                    </div>
                    <Button
                      type="button"
                    ...
                    <div>
                      ...
                      <Select
                        ...
                        onChange={(event) =>
                          setAiForm((current) => ({
                            ...current,
                            learningLevel: event.target.value as
                              | "Simple"
                              | "Moderate"
                              | "Advanced",
                          }))
                    ...
                    <div>
                      ...
                      <Input
                        ...
                        onChange={(event) =>
                          setAiForm((current) => ({
                            ...current,
                            curriculumStyle: event.target.value,
                          }))
                        }
                      ...
                      <div className="mb-4 flex items-center justify-between gap-4">
                        <div>
                          <p className="text-base font-semibold text-slate-900">
                            Generated lesson note ready to insert
                          </p>
                          <p className="mt-1 text-sm text-slate-600">
                            You can review the generated draft and insert it
                            into the note form.
                          </p>
                        </div>
                      </div>
                ...
                <div className="flex flex-col gap-4 sm:flex-row sm:items-center sm:justify-between">
                  <div>
                    <p className="font-semibold">
                      You have an unsaved local draft from{" "}
                      {formatRecoveryTime(localDraft.savedAt)}.
                    </p>
                    <p className="mt-1 text-sky-800">
                      Restore it to continue where you stopped, or discard it
                      and keep the current form.
                    </p>
                  </div>
            ...
            >
                ...
                <div>
                  ...
                  <Select
                    {...register("academic_session_id", {
                      required: !id ? "Academic session is required" : false,
                    })}
                    disabled={noteLocked}
                  >
              ...
              <div className="grid gap-4 lg:grid-cols-2">
                ...
                <div>
                  ...
                  <Textarea
                    ...
                    placeholder="List any resources or materials"
                    disabled={noteLocked}
                  />
                  {errors.materials && (
                    <p className="mt-1 text-sm text-rose-600">
              ...
              <div className="grid gap-4 lg:grid-cols-2">
                ...
                <div>
                  ...
                  <Textarea
                    ...
                    placeholder="What tasks will learners complete?"
                    disabled={noteLocked}
                  />
                  {errors.assignment && (
                    <p className="mt-1 text-sm text-rose-600">
              ...
              <div>
                ...
                <Textarea
                  {...register("teachers_presentation", {
                    required: "Teacher's presentation is required",
                  })}
                  placeholder="Describe how you will present this lesson to the class"
                  disabled={noteLocked}
                />
                {errors.teachers_presentation && (
                  <p className="mt-1 text-sm text-rose-600">
                    {errors.teachers_presentation.message}
                  </p>
                )}
              </div>
              ...
              <div className="flex flex-col gap-3 border-t border-slate-200 pt-5 sm:flex-row sm:items-center sm:justify-between">
                <div className="flex flex-col gap-3 sm:flex-row">
                  <Button
                    ...
                    variant="secondary"
                    onClick={latestNote}
                    disabled={duplicateLoading || noteLocked}
                  >
