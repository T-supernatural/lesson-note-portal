...
const AdminReviewPage = () => {
  ...
  useEffect(() => {
    ...
    fetchNoteById(id)
      .then(async (data) => {
        ...
        if (data.academic_session_id) {
          fetchAcademicSessions().then((sessions) => {
            setSessionName(
              sessions.find(
                (session) => session.id === data.academic_session_id,
              )?.name || "Unassigned",
            );
          });
        }
      ...
      .catch(() => {
        setError("Unable to load lesson note.");
        toast.error("Unable to load lesson note");
      })
  ...
  const submitReview = async (
    ...
    try {
      ...
      setNote((current: any) =>
        current
          ? { ...current, status, admin_comment: values.admin_comment }
          : current,
      );
    ...
    return (
      <div className="min-h-screen bg-slate-50 px-4 py-6">
        <div className="mx-auto max-w-6xl">
          <InlineError message={error || "Lesson note not found."} />
        </div>
      </div>
    );
  }
      ...
      <div className="mx-auto max-w-6xl">
        ...
        <div className="grid gap-6 lg:grid-cols-[1.3fr_0.7fr]">
          <div className="rounded-[32px] border border-slate-200 bg-white p-8 shadow-soft">
            <PageHeader
              title={note.topic}
              description={`Submitted by ${teacherName} • ${note.subject} • ${note.term} • Week ${note.week}`}
            />
          ...
          <div className="rounded-[32px] border border-slate-200 bg-white p-8 shadow-soft">
            ...
            {reviewsError ? (
              <div className="mt-6">
                <InlineError message={reviewsError} />
              </div>
            ) : null}
            ...
            <form
              ...
              onSubmit={handleSubmit((data) => submitReview("rejected", data))}
            >
              <div>
