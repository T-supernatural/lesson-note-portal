...
const AdminMissingNotesPage = () => {
  ...
  const { register, handleSubmit, reset } = useForm<ReportFilters>({
  ...
  useEffect(() => {
    ...
    Promise.all([
      ...
      .then(([teacherData, sessionData, deadlineData]) => {
        setTeachers(teacherData);
        setSessions(sessionData);
        setDeadlines(deadlineData);
      })
      .catch(() => {
        setError("Unable to load report options.");
        toast.error("Unable to load report options");
      })
  ...
  const runReport = async (filters: ReportFilters) => {
    ...
    try {
      ...
      setError("Unable to generate the missing-notes report.");
  ...
  const selectedSession =
    sessions.find((session) => session.id === reportFilters.session)?.name ||
    "all sessions";
  const reportDeadline = deadlines.find(
    (deadline) =>
      deadline.is_active &&
      ...
      (!reportFilters.day ||
        !deadline.lesson_day ||
        deadline.lesson_day === reportFilters.day),
  );
  const isOverdue = reportDeadline
    ? new Date(reportDeadline.due_at).getTime() < Date.now()
    : false;
        ...
        <div className="mx-auto max-w-6xl">
          <div className="mb-6 flex flex-col gap-4 sm:flex-row sm:items-center sm:justify-between">
            <div>
              ...
              <p className="mt-2 text-sm text-slate-600">
                Identify active teachers without a submitted, approved, or
                rejected note for a selected schedule.
              </p>
            </div>
          ...
          >
            <div className="mb-4 flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between">
              <div>
                ...
                <p className="mt-1 text-sm text-slate-500">
                  Choose the schedule teachers are expected to complete.
                </p>
              </div>
              <div className="flex gap-3">
                ...
                <Button type="submit" disabled={reportLoading || loading}>
            ...
            <div className="grid gap-4 md:grid-cols-2 lg:grid-cols-3">
              ...
              <label className="text-xs font-semibold uppercase tracking-[0.18em] text-slate-500">
                Subject
                <Input {...register("subject")} placeholder="Any subject" />
              </label>
              <label className="text-xs font-semibold uppercase tracking-[0.18em] text-slate-500">
                Week
          ...
          {error ? (
            ...
            <LoadingState label="Loading report options..." />
          ) : !hasRun ? (
            <EmptyState
              title="Report not generated"
              subtitle="Choose a session, term, week, or other scope and generate the report."
            />
          ) : (
            <>
              {reportDeadline ? (
                <div
                  className={`mb-4 rounded-2xl border p-4 text-sm ${isOverdue ? "border-rose-200 bg-rose-50 text-rose-900" : "border-sky-200 bg-sky-50 text-sky-900"}`}
                >
                  {isOverdue
                    ? "This submission scope is overdue."
                    : "Submission deadline"}{" "}
                  Due {new Date(reportDeadline.due_at).toLocaleString()}.
                </div>
              ) : null}
              {missingTeachers.length === 0 ? (
                <EmptyState
                  title="All active teachers have submitted notes"
                  subtitle={`No missing submissions found for ${selectedSession}.`}
                />
              ) : (
                <div className="rounded-[32px] border border-amber-200 bg-amber-50 p-6">
                  <PageHeader
                    title={`${missingTeachers.length} missing submission${missingTeachers.length === 1 ? "" : "s"}`}
                    description="These active teachers have no non-draft note matching the selected scope."
                  />
                  <div className="mt-4 grid gap-3">
                    {missingTeachers.map((teacher) => (
                      <div
                        key={teacher.id}
                        className="flex flex-col gap-2 rounded-2xl border border-amber-200 bg-white p-4 sm:flex-row sm:items-center sm:justify-between"
                      >
                        <div>
