...
const BottomTabBar = ({ items }: { items: BottomTab[] }) => (
  <nav className="fixed bottom-0 left-0 right-0 z-50 grid border-t border-slate-200 bg-white/95 px-2 py-2 pb-[calc(0.5rem+env(safe-area-inset-bottom))] backdrop-blur md:hidden print:hidden" style={{ gridTemplateColumns: `repeat(${items.length}, minmax(0, 1fr))` }} aria-label="Mobile navigation">
