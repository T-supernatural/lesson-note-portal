      ...
      <div className="flex flex-col gap-4 sm:flex-row sm:items-center sm:justify-between">
        <div className="flex-1">
          <div className="flex items-center gap-2 text-slate-500">
            ...
            <span className="text-xs uppercase tracking-[0.24em]">Week {note.week} • {note.subject} • {note.academic_session_id ? 'Session assigned' : 'Session unassigned'}</span>
          </div>
          <h2 className="mt-2 text-lg font-semibold text-slate-900">{note.topic}</h2>
          <p className="mt-2 text-sm text-slate-600">{note.class_level} • {note.term} • {note.lesson_day || 'Unspecified day'}</p>
